...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, position: tuple, direction: tuple, patrol_points: list):
        ...
        self.attack_close_timer = 0.3 # seconds
    ...
    def _build_image(self) -> pygame.Surface:
        ...
        if self.icon != None:
            self.icon_surface = pygame.image.load(resource_path(f"assets/icons/{self.icon}.png"))
    ...
    def chase(self, dt):
        ...
        if self.attack_timer <= 0:
            ...
            if dist_to_player < close_dist:
                self.attack_timer = self.attack_close_timer
            else:
                self.attack_timer = 1.0 # seconds
