...
class Settings:
    ...
    def load_config(self):
        if os.path.exists("config.json"):
            ...
            if len(config) == 3:
                ...
                settings.width, settings.height = config["resolution"]
                settings.is_fullscreen = config["is_fullscreen"]
    ...
    def scale_true_rect(self, rect: pygame.Rect) -> pygame.Rect:
        """Base-res Rect  →  true pixel Rect."""
        return pygame.Rect(
            round(rect.x      * self.scale_true_x),
            round(rect.y      * self.scale_true_y),
            round(rect.width  * self.scale_true_x),
            round(rect.height * self.scale_true_y,)
        )
