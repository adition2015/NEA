...
class PopUpMenu(Menu):
    ...
    def __init__(self):
        ...
        self.font_lg = pygame.font.SysFont("Segoe UI", 28)
...
class StartMenu(StaticMenu):
    def _build(self):
        self.title_font = pygame.font.SysFont("Segoe UI", int(36*settings.scale_true_x))
        ...
        self.play_rect = settings.scale_true_rect(self.play_rect)
        ...
        self.settings_rect = pygame.Rect(0, 0, btn_w, btn_h)
        self.settings_rect = settings.scale_true_rect(self.settings_rect)
        self.settings_rect.center = (int(pos2.x), int(pos2.y))
        self.buttons["settings"] = self.settings_rect

        pos3 = settings.to_true_screen(pygame.Vector2(540, 500))
        self.exit_rect = pygame.Rect(0, 0, btn_w, btn_h)
        self.exit_rect = settings.scale_true_rect(self.exit_rect)
        self.exit_rect.center = (int(pos3.x), int(pos3.y))
        self.buttons["exit"] = self.exit_rect

    def _draw_content(self, surface):
        title = self.title_font.render("STEALTH GAME", True, (220, 220, 200))
        ...
        pygame.draw.rect(surface, (250, 95, 85), self.exit_rect, border_radius=8)
        ...
        surface.blit(font.render("Exit", True, (220, 220, 220)),
                     font.render("Exit", True, (0,0,0)).get_rect(center=self.exit_rect.center))
...
class SettingsMenu(StaticMenu):
    ...
    def _build(self):
        ...
        self.button_font = pygame.font.SysFont("Segoe UI", 18)
    ...
    def _apply(self):
        ...
        with open("config.json", "w") as f:
            json.dump({"fps": settings.FPS,
                        "resolution": (settings.width, settings.height), 
                        "is_fullscreen": settings.is_fullscreen}, f)

        settings.init_resolution()

        flags = pygame.FULLSCREEN if settings.is_fullscreen else 0
        pygame.display.set_mode(settings.res, flags)
...
class Menus:
    ...
    def show_popup(self, popup: PopUpMenu):
        if isinstance(popup, PauseMenu) and self.pop_up != None:
            return
        self.pop_up = popup
        pass
