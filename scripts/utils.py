...
def level_creation(fields: dict, destination: str):
    ...
    with open(destination, "r") as f:
        ...
        existing = list(json.loads(content)) if content else []
    if count > 0:
...
def load_level(level_id: int) -> dict:
    path = resource_path(f"levels/level_{level_id:02d}.json")
...
#level_creation(hiding_spot_fields, "levels/level_02_hiding_spots.json")
...
def draw_debug(surface, data: dict, pos=(10, 10), size=15, colour=(0, 255, 0)):
    ...
    if size not in _debug_fonts:
        _debug_fonts[size] = pygame.font.SysFont("Segoe UI", size)
...
def merge_level(level_id): 
    ...
    merged = {
        ...
        "player": {"position": [150, 500], "direction": [0, -1]},
    ...
    with open(resource_path(f"levels/level_{n}.json"), "w") as f:
...
# helper function for maintaining asset hierarchy:

def resource_path(relative_path):
    """Get the correct path whether running as script or bundled."""
...
# For any image loading, use this function over the path.
#merge_level(2)